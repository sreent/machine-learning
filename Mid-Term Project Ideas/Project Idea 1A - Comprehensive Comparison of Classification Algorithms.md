...
## Aim
To compare several core classification algorithms covered in the course—k-Nearest Neighbours (kNN), Naive Bayes (Gaussian), Logistic Regression, and Decision Tree classification—on the Breast Cancer Wisconsin Diagnostic dataset (from scikit-learn) in order to:
...
At least one of these algorithms (likely kNN or Gaussian Naive Bayes) will be implemented **from scratch** (using only Python and NumPy), with the others possibly using **scikit-learn** for comparison.

---

## Datasets

This project will use the **Breast Cancer Wisconsin Diagnostic** dataset from scikit-learn (`sklearn.datasets.load_breast_cancer()`).

This dataset is small, relatively clean, and supports binary classification (benign vs malignant), making it ideal for focusing on the algorithms themselves rather than extensive data preprocessing, while still allowing the use of rich evaluation metrics such as precision, recall, and F1 score. Because false negatives (malignant tumours predicted as benign) are particularly costly in this context, the analysis will pay special attention to recall for the malignant class and the trade-off between precision and recall.
...
### 4. Cross-Validation
- Because this dataset is of **moderate size**, perform **k-fold cross-validation (e.g. k = 5)** on the **training set** only.
...
- Discuss the trade-offs by:
  - Identifying which algorithm performs best overall based on the chosen test metrics (e.g. F1 score for the malignant class and overall accuracy) and explaining *why* it performs best in terms of its assumptions and decision boundaries.
...
3. **Background**  
   Explain each algorithm (kNN, Naive Bayes, Logistic Regression, Decision Tree) and the optimisation method **gradient descent** as the optimisation method typically used for training Logistic Regression:  
   - main idea,  
...
6. **Evaluation**  
   - Critical discussion of:  
     ...
     - implications of false negatives and the importance of malignant-class recall and the precision–recall trade-off.  
